/* ============================================================
   ...
   4. 明文永遠不寫進任何儲存。解鎖後的內容放在這裡的記憶體 Map，
...
(function () {
  ...
  function changePassword(oldPassword, newPassword, cards) {
    return rewrapAll(oldPassword, newPassword, cards, false);
  }

  /**
   * 用復原金鑰重設主密碼。
   *
   * 與 changePassword 的差別在**卡片包裹要從哪裡取得金鑰**：
   * 每張卡片的 pwd 包裹是用主密碼包的，復原金鑰解不開它。
   * 這條路必須走主金鑰 → 卡片的 rec 包裹，才拿得到卡片金鑰去重新包裹。
   */
  function resetPassword(recoveryKey, newPassword, cards) {
    return rewrapAll(recoveryKey, newPassword, cards, true);
  }

  function rewrapAll(secret, newPassword, cards, viaRecovery) {
    if (!subtle) return Promise.reject(new Error('這個環境不支援加密'));
    var out = {}, masterKey;
    return getMasterKey(secret).then(function (master) {
      masterKey = master;
      ...
      return (cards || []).reduce(function (chain, c) {
        return chain.then(function () {
          ...
          var getCardKey = viaRecovery
            ? unwrapWithKey(c.vault.rec, masterKey)
            : unwrapWithPassword(c.vault.pwd, secret);
          return getCardKey
  ...
  /**
   * 沒有任何加密卡片時，主密碼留著沒有意義，反而是陷阱：
   * 下次新建卡片會沿用一組使用者早就忘了的舊密碼，畫面上卻只說
   * 「主密碼不正確」。歸零就一起清掉。
   */
  ...
  window.Vault = {
    ...
    dropMasterIfUnused: dropMasterIfUnused,
    getPlain: getPlain,
    setPlain: setPlain,
    reset: reset,
